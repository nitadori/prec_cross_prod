...
template <typename F>
F tp_fma(F a, F b, F c);

template<>
float tp_fma(float a, float b, float c){ return ::fmaf(a, b, c); }

template<>
double tp_fma(double a, double b, double c){ return ::fma(a, b, c); }

template <typename F>
F prec_cross_prod_inner(F ux, F uy, F vx, F vy){
	F p = ux * vy;
	F r = tp_fma(ux, vy, -p);
